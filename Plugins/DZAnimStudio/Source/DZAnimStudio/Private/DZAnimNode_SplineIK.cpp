...
#include "Core/DZSplineIK.h"

FDZAnimNode_SplineIK::FDZAnimNode_SplineIK()
	: BoneAxis(EDZSplineBoneAxis::X)
...
void FDZAnimNode_SplineIK::GatherDebugData(FNodeDebugData& DebugData)
{
...
void FDZAnimNode_SplineIK::OnInitializeAnimInstance(const FAnimInstanceProxy* InProxy, const UAnimInstance* InAnimInstance)
{
...
struct FDZSplineIKScratchArea : public TThreadSingleton<FDZSplineIKScratchArea>
{
...
void FDZAnimNode_SplineIK::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	if (CachedBoneReferences.Num() > 0)
	{
		...
		TArray<FTransform>& InTransforms = FDZSplineIKScratchArea::Get().InTransforms;
		TArray<FTransform>& OutTransforms = FDZSplineIKScratchArea::Get().OutTransforms;
		TArray<FCompactPoseBoneIndex>& CompactPoseBoneIndices = FDZSplineIKScratchArea::Get().CompactPoseBoneIndices;
		InTransforms.Reset();
		OutTransforms.Reset();
		...
		{
			const FDZSplineIKCachedBoneData& BoneData = CachedBoneReferences[BoneIndex];
		...
		DZAnimationCore::DZSolveSplineIK(InTransforms, TransformedSpline.Position, TransformedSpline.Rotation, 
			TransformedSpline.Scale, TotalSplineAlpha, TotalSplineLength, 
			FDZFloatMapping::CreateRaw(this, &FDZAnimNode_SplineIK::GetTwist, TotalSplineAlpha), 
			Roll, Stretch, Offset, (EAxis::Type)BoneAxis, 
			FDZFindParamAtFirstSphereIntersection::CreateRaw(this, &FDZAnimNode_SplineIK::FindParamAtFirstSphereIntersection), 
			CachedOffsetRotations, CachedBoneLengths, OriginalSplineLength, OutTransforms);
...
bool FDZAnimNode_SplineIK::IsValidToEvaluate(const USkeleton* Skeleton, const FBoneContainer& RequiredBones)
{
	// If any bone references are valid, evaluate.
	if (CachedBoneReferences.Num() > 0)
	{
		for (FDZSplineIKCachedBoneData& CachedBoneData : CachedBoneReferences)
		{
...
void FDZAnimNode_SplineIK::InitializeBoneReferences(const FBoneContainer& RequiredBones)
{
...
FTransform FDZAnimNode_SplineIK::GetTransformedSplinePoint(int32 TransformIndex) const
{
...
FTransform FDZAnimNode_SplineIK::GetControlPoint(int32 TransformIndex) const
{
...
void FDZAnimNode_SplineIK::SetControlPoint(int32 TransformIndex, const FTransform& InTransform)
{
	if (ControlPoints.IsValidIndex(TransformIndex))
	{
...
void FDZAnimNode_SplineIK::SetControlPointLocation(int32 TransformIndex, const FVector& InLocation)
{
	if (ControlPoints.IsValidIndex(TransformIndex))
	{
...
void FDZAnimNode_SplineIK::SetControlPointRotation(int32 TransformIndex, const FQuat& InRotation)
{
	if (ControlPoints.IsValidIndex(TransformIndex))
	{
...
void FDZAnimNode_SplineIK::SetControlPointScale(int32 TransformIndex, const FVector& InScale)
{
	if (ControlPoints.IsValidIndex(TransformIndex))
	{
...
void FDZAnimNode_SplineIK::TransformSpline()
{
...
float FDZAnimNode_SplineIK::FindParamAtFirstSphereIntersection(const FVector& InOrigin, float InRadius, int32& StartingLinearIndex)
{
...
//收集
void FDZAnimNode_SplineIK::GatherBoneReferences(const FReferenceSkeleton& RefSkeleton)
{
	//清理缓存
	CachedBoneReferences.Reset();
	int32 vStartBoneIndex = RefSkeleton.FindBoneIndex(StartBone.BoneName);
	int32 vEndBoneIndex = RefSkeleton.FindBoneIndex(EndBone.BoneName);
	if (vStartBoneIndex != INDEX_NONE && vEndBoneIndex != INDEX_NONE)
	{
		int vBoneIndex = vEndBoneIndex;
		while (vBoneIndex != vStartBoneIndex)
		{
			if (vBoneIndex == INDEX_NONE)
			{
				CachedBoneReferences.Reset();
				break;
			}
			FName vBoneIndexName = RefSkeleton.GetBoneName(vBoneIndex);
			CachedBoneReferences.EmplaceAt(0, vBoneIndexName, vBoneIndex);

			vBoneIndex = RefSkeleton.GetParentIndex(vBoneIndex);
		}
		if (CachedBoneReferences.Num())
		{
			FName vBoneIndexName = RefSkeleton.GetBoneName(vStartBoneIndex);
			CachedBoneReferences.EmplaceAt(0, vBoneIndexName, vStartBoneIndex);
...
void FDZAnimNode_SplineIK::BuildBoneSpline(const FReferenceSkeleton& RefSkeleton)
{
	if (CachedBoneReferences.Num() > 0)
	{
		...
		for (int32 BoneIndex = 0; BoneIndex < CachedBoneReferences.Num(); BoneIndex++)
		{
			...
			if (BoneIndex > 0)
			{
				FDZSplineIKCachedBoneData& BoneData = CachedBoneReferences[BoneIndex];
				const FTransform& Transform = ComponentSpaceTransforms[BoneData.RefSkeletonIndex];

				const FDZSplineIKCachedBoneData& ParentBoneData = CachedBoneReferences[BoneIndex - 1];
...
float FDZAnimNode_SplineIK::GetTwist(float InAlpha, float TotalSplineAlpha)
{
