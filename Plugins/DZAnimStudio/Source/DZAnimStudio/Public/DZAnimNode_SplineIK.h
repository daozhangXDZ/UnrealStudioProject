...
#include "DZAnimNode_SplineIK.generated.h"
 ...
 */
UENUM()
enum class EDZSplineBoneAxis : uint8
{
	X = 1,
	Y = 2,
	Z = 3,
...
USTRUCT()
struct FDZSplineIKCachedBoneData
{
	GENERATED_BODY()

		FDZSplineIKCachedBoneData()
		: Bone(NAME_None)
		, RefSkeletonIndex(INDEX_NONE)
	{}

	FDZSplineIKCachedBoneData(const FName& InBoneName, int32 InRefSkeletonIndex)
		: Bone(InBoneName)
...
USTRUCT()
struct DZANIMSTUDIO_API FDZAnimNode_SplineIK : public FAnimNode_SkeletalControlBase
{
	GENERATED_BODY()
...
private:
	...
	UPROPERTY()
		TArray<FDZSplineIKCachedBoneData> CachedBoneReferences;
