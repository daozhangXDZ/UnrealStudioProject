...
bool UVRIKSkeletalMeshTranslator::UpdateSkeletonBonesSetup()
{
	...
	//默认值处理，当前左右手臂长度默认为原始模型的对应长度
	{
		BodySetup.fCurrLeftUpperarmLength = BodySetup.fUpperarmLength;
		BodySetup.fCurrRightUpperarmLength = BodySetup.fUpperarmLength;
		BodySetup.fCurrLeftForearmLength = BodySetup.fForearmLength;
		BodySetup.fCurrRightForearmLength = BodySetup.fForearmLength;
		BodySetup.DefaultLeftHandScale = tPalmLeft.GetScale3D();
		BodySetup.DefaultRightHandScale = tPalmRight.GetScale3D();
	}
	...
	if (VRIKSolver) {
		...
		if (bRescaleMesh && VRIKSolver->IsBodyCalibrated()) {
			...
			BodySetup.fForearmLength = BodySetup.MeshForearmLength * BodySetup.MeshScale.X;
			{
				...
				BodySetup.fCurrRightUpperarmLength = BodySetup.fUpperarmLength;
				BodySetup.fCurrLeftForearmLength = BodySetup.fForearmLength;
				BodySetup.fCurrRightForearmLength = BodySetup.fUpperarmLength;
			}
		}
		else {
			if (BodyMesh) {
...
bool UVRIKSkeletalMeshTranslator::GetLastPose(FPoseSnapshot& ReturnPoseValue)
{
	...
	**********************************************************************/
	for (int32 ComponentSpaceIdx = 1; ComponentSpaceIdx < NumSpaceBases; ++ComponentSpaceIdx)
	{
		...
		if (bHandsIKLateUpdate) {
			...
			if (BoneName == UpperarmRight) {
				...
				CalculateTwoBoneIK(
					...
					BodySetup.fCurrRightUpperarmLength, BodySetup.fCurrRightForearmLength,
				...
				//FString LogPalmLeftPos = FString::Printf(TEXT("fCurrRightForearmLength = %s"), *FString::SanitizeFloat(BodySetup.fCurrRightForearmLength));
				//FString LogPalmLeftPos = FString::Printf(TEXT("CapturedBodyRaw.HandRight = %s"), *CapturedBody[LowerarmRight].ToString());
				//GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Red, LogPalmLeftPos);
				...
				CalculateTwoBoneIK(
					...
					BodySetup.fCurrLeftUpperarmLength, BodySetup.fCurrLeftForearmLength,
		...
		if (bBoneHasEvaluated && tr_bone.Rotator() != FRotator::ZeroRotator) {
			...
			else if (BoneName == PalmRight || BoneName == PalmLeft)
			{
		...
		{
			ReturnPoseValue.LocalTransforms[ComponentSpaceIdx].SetLocation(CapturedSkeletonR[0].InverseTransformPosition(CapturedBody[LowerarmRight].GetTranslation()));
			CapturedSkeletonR[1] = ReturnPoseValue.LocalTransforms[ComponentSpaceIdx] * CapturedSkeletonR[0];
			DrawDebugBox(GetWorld(), CapturedSkeletonR[1].GetLocation(), FVector(2, 2, 2), FColor::Purple, true, 0.1f, 0, 2);

		}
		else if (BoneName == PalmRight)
		{
			//uPDATE hand loc trans because CalcTorsoIK will use FK
			/*FString LogPalmLeftPos = FString::Printf(TEXT("PalmLeft = %s"), *(CapturedBodyRaw.HandRight.GetTranslation().ToString()));
			GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Red, LogPalmLeftPos);*/
			ReturnPoseValue.LocalTransforms[ComponentSpaceIdx].SetLocation(CapturedSkeletonR[1].InverseTransformPosition(CapturedBodyRaw.HandRight.GetTranslation()));
		...
		{
			ReturnPoseValue.LocalTransforms[ComponentSpaceIdx].SetLocation(CapturedSkeletonL[0].InverseTransformPosition(CapturedBody[LowerarmLeft].GetTranslation()));
			CapturedSkeletonL[1] = ReturnPoseValue.LocalTransforms[ComponentSpaceIdx] * CapturedSkeletonL[0];
		}
		else if (BoneName == PalmLeft)
		{
			//uPDATE hand loc trans because CalcTorsoIK will use FK
			/*FString LogPalmLeftPos = FString::Printf(TEXT("PalmLeft = %s"), *(CapturedBodyRaw.HandLeft.GetTranslation().ToString()));
			GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Red, LogPalmLeftPos);*/
			ReturnPoseValue.LocalTransforms[ComponentSpaceIdx].SetLocation(CapturedSkeletonL[1].InverseTransformPosition(CapturedBodyRaw.HandLeft.GetTranslation()));
	...
	**********************************************************************/
	...
	for (int32 ComponentSpaceIdx = 2; ComponentSpaceIdx < NumSpaceBases; ++ComponentSpaceIdx)
	{
		...
		//BaiPao_XD Hand Scale Revert to Default World Scale
		float vScaleProTemp = BodySetup.MeshScale.Z <= 0.0f ? 0.001f : BodySetup.MeshScale.Z;
		FVector vReverveWorldScale = FVector(1.0f / vScaleProTemp, 1.0f / vScaleProTemp, 1.0f / vScaleProTemp);
		if (BoneName == PalmLeft)
		{
		
			ReturnPoseValue.LocalTransforms[ComponentSpaceIdx].SetScale3D(BodySetup.DefaultLeftHandScale * vReverveWorldScale);
		}
		else if (BoneName == PalmRight)
		{
			ReturnPoseValue.LocalTransforms[ComponentSpaceIdx].SetScale3D(BodySetup.DefaultRightHandScale * vReverveWorldScale);
		}
	}
...
bool UVRIKSkeletalMeshTranslator::UpdateBonesState()
{
	...
	//将IKSolver计算出的手部长度进行匹配
	{
		BodySetup.fCurrLeftUpperarmLength = VRIKSolver->fUpperarm_Left_Length;
		BodySetup.fCurrRightUpperarmLength = VRIKSolver->fUpperarm_Right_Length;
		BodySetup.fCurrLeftForearmLength = VRIKSolver->fForearm_Left_Length;
		BodySetup.fCurrRightForearmLength = VRIKSolver->fForearm_Right_Length;
		/*FString LogPalmLeftPos = FString::Printf(TEXT("fCurrRightUpperarmLength = %s"), *FString::SanitizeFloat(BodySetup.fCurrLeftUpperarmLength));
		GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Red, LogPalmLeftPos);*/
	}
	// twists (2018/06/28)
