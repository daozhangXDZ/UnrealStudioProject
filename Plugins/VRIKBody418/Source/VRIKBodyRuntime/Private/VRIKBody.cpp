...
void UVRIKBody::BeginPlay()
{
	...
	//初始化默认前后手臂长度（左右手）
	fUpperarm_Right_Length = UpperarmLength;
	fForearm_Right_Length = HandLength - fUpperarm_Right_Length;

	fUpperarm_Left_Length = UpperarmLength;
	fForearm_Left_Length = HandLength - fUpperarm_Right_Length;
...
{
	float UpperarmLengthScaleFinal = UpperarmLength;
	...
	if (DirectSize > HandLength) {
		...
		{
			if (DirectSize > HandScaleMaxLength)
			{
				DirectSize = HandScaleMaxLength;
			}
			UpperarmLengthScaleFinal = UpperarmLength * (DirectSize / HandLength);
			ForeSize = DirectSize - UpperarmLengthScaleFinal;
	...
	if (Hand == EControllerHand::Right)
	{
		fUpperarm_Right_Length = UpperarmLengthScaleFinal;
		fForearm_Right_Length = ForeSize;
	}
	else
	{
		fUpperarm_Left_Length = UpperarmLengthScaleFinal;
		fForearm_Left_Length = ForeSize;
		/*FString LogPalmLeftPos = FString::Printf(TEXT("fUpperarm_Left_Length = %s"), *FString::SanitizeFloat(fUpperarm_Left_Length));
		GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Red, LogPalmLeftPos);*/
	}
	// correct hand length ---end
	...
	CalculateTwoBoneIK(ShoulderEnd, HandLoc, JointTarget, UpperarmLengthScaleFinal, ForeSize, UpperarmTransform, ForearmTransform, side);
...
/**
 * Scale By Body Height and Mesh Height
 */
void UVRIKBody::CalibrateSkeletonByBodyHeight()
{
	nModifyHeightState = HEIGHT_STABLE;
	...
	/*BodyWidth = FVector::Dist(CalI_ControllerR.GetTranslation(), CalI_ControllerL.GetTranslation()) - 5.f;*/
	CharacterHeight =  CalT_Head.GetTranslation().Z + 3.f - GroundZ - FMath::Sin(HeadPitch) * 3.f;
	CharacterHeightClear = CalT_Head.GetTranslation().Z - GroundZ;

	float vScale_ControlHeight_SklemeshtalHeight = CharacterHeight / SkeletoneDefaultParams.CharacterHeight;
	bCalibratedT = bCalibratedI = true;

	BodyWidth =					vScale_ControlHeight_SklemeshtalHeight * SkeletoneDefaultParams.fBodyWidth;
	CharacterHeight =			vScale_ControlHeight_SklemeshtalHeight * SkeletoneDefaultParams.CharacterHeight;
	LegsLength =				vScale_ControlHeight_SklemeshtalHeight * SkeletoneDefaultParams.LegsLength;
	NeckToHeadsetOffset =		vScale_ControlHeight_SklemeshtalHeight * SkeletoneDefaultParams.vNeckToHeadsetOffset;
	SpineLength =				vScale_ControlHeight_SklemeshtalHeight * SkeletoneDefaultParams.fSpineLength;
	HandLength =				vScale_ControlHeight_SklemeshtalHeight * SkeletoneDefaultParams.fHandLength;
	CharacterHeightClear =		vScale_ControlHeight_SklemeshtalHeight * SkeletoneDefaultParams.CharacterHeightClear;
	ArmSpanClear =				vScale_ControlHeight_SklemeshtalHeight * SkeletoneDefaultParams.ArmSpanClear;

	HandScaleMaxLength =		vScale_ControlHeight_SklemeshtalHeight * SkeletoneDefaultParams.HandScaleMaxLength;
	nModifyHeightState = HEIGHT_STABLE;
	UpperarmLength =			vScale_ControlHeight_SklemeshtalHeight * SkeletoneDefaultParams.UpperarmLength;
	fUpperarm_Right_Length =	UpperarmLength;
	fForearm_Right_Length =		HandLength - fUpperarm_Right_Length;
	...
	ThighLength =				vScale_ControlHeight_SklemeshtalHeight * SkeletoneDefaultParams.ThighLength;
	bResetFeet = true;
